...
export async function makePizzas(order: PizzaOrder): Promise<void> {
  log.info(`Starting to make pizzas for order ${order.orderNumber}`);

  for (const pizza of order.items) {
    log.info(`Making pizza: ${pizza.description}`);
    // Simulate the time taken to make a pizza.
  }

  log.info(`All pizzas for order ${order.orderNumber} are ready!`);
}

export async function deliverPizzas(order: PizzaOrder): Promise<void> {
  log.info(`Starting delivery ${order.orderNumber} to ${order.address}`);
  // Simulate the time to make delivery
  log.info(`Order ${order.orderNumber} delivered.`);
}
