import { Address, Bill, Distance, PizzaOrder, OrderConfirmation } from './shared';
...
export async function makePizzas(order: PizzaOrder): Promise<void> {
  const context = activity.Context.current();
  context.log.info(`Starting to make pizzas for order ${order.orderNumber}`);

  for (const pizza of order.items) {
    context.log.info(`Making pizza: ${pizza.description}`);
    // Simulate the time taken to make a pizza.
  }

  context.log.info(`All pizzas for order ${order.orderNumber} are ready!`);
}

export async function deliverPizzas(order: PizzaOrder): Promise<void> {
  const context = activity.Context.current();
  context.log.info(`Starting delivery ${order.orderNumber} to ${order.address}`);
  // Simulate the time to make delivery
  context.log.info(`Order ${order.orderNumber} delivered.`);
}
